    ...
    for car in cars:
        try:
            ...
            car_data = {
                ...
                # 'inspection_image':get_matching_values(df,car.find(class_='text_style7').text,'inspection_image'),
                'price':car.find(class_='i_comm_main_txt2').text,
...
with open(f'/Volumes/ahmed/car_auction_data/autohub/text_data/autohub_detail_data_2025-05-27-13.txt') as f:
    data = f.read()
    parese_data(data=data,date=date.today(),df=False,df_path='',output_path=f'/Volumes/ahmed/car_auction_data/autohub/jsion_data/detail_json_{TODAY}.json')
